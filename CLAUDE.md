...
make rss                  # Build site and generate RSS/Atom feeds
```
...
make build                       # Same as above, using Makefile
```

### Cleanup
```bash
make clean                # Remove .venv and site directories
```
...
- **`mkdocs.ci.yml`** - Extends base via `INHERIT`, adds production plugins (minify, search)
...
**RSS & Atom Feeds** (`scripts/generate-rss.py`)
- Uses `feedgen` library to generate both RSS 2.0 and Atom 1.0 feeds
- Generates `site/feed.xml` (RSS) and `site/feed.atom` (Atom)
- Includes full post content (HTML extracted from built site)
- Lists most recent 20 posts in reverse chronological order
- Run automatically during CI build, or manually with `make rss`

**CRITICAL**: Never manually edit index files or RSS feed. They are completely regenerated by scripts.
...
- `scripts/generate-rss.py` - RSS feed generator (site/feed_rss_created.xml)
